...
impl DatabasePort for SledDatabaseAdapter {
    /// Inserts a key-value pair into the database through the adapter.
    ...
    /// Retrieves a value from the database by key through the adapter.
    fn get(&self, key: &[u8]) -> Result<Option<IVec>, Box<dyn Error>> {
        let value = self.db.get(key)?;
        Ok(value)
    }

    /// Removes a key-value pair from the database through the adapter.
