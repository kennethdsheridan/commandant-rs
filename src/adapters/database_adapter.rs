...
impl DatabasePort for DatabaseAdapter {
    ...
    async fn insert(
        ...
        let thing = thing(&thing_id).map_err(|e| {
            Box::new(ParseThingError(format!("Failed to create Thing: {:?}", e)))
    ...
    async fn get(&self, key: &str) -> Result<Option<String>, Box<dyn Error + Send + Sync>> {
        ...
        let record: Option<serde_json::Value> = self.db.select(thing).await?;
    ...
    async fn remove(&self, key: &str) -> Result<Option<String>, Box<dyn Error + Send + Sync>> {
        let thing: Thing = ("key_value", key).into();
        let deleted: Option<serde_json::Value> = self.db.delete(thing).await?;
